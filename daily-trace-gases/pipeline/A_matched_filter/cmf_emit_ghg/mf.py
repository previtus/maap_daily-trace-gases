#! /usr/bin/env python
# SOURCE > https://github.com/emit-sds/emit-ghg/blob/main/parallel_mf.py
#
...
# Authors: Vit Ruzicka (simplified for codebase, removed usage of ray)
...
from pipeline.A_matched_filter.cmf_emit_ghg.s_utils import envi_header, write_bil_chunk, SerialEncoder
import pipeline.A_matched_filter.cmf_emit_ghg.spec_io as spec_io
import json

import logging

def main(input_args=None, override={}):
    ...
    parser.add_argument('--loglevel', type=str, default='WARNING', help='logging verbosity')
    ...
    # print("mf args:", input_args)
    args = parser.parse_args(input_args)

    for k in override.keys():
        setattr(args, k, override[k])
        print("setting", k, "to", getattr(args, k))
    ...
    if args.wavelength_range is None:
        logging.error('wavelength range - could not set a default active range')
        sys.exit(0)
    else:
    ...
    for _ce, ce in enumerate(chunk_edges[:-1]):
        ...
        for col in range(output_shape[2]):
            ret = mf_one_column(col, radiance, absorption_coefficients, active_wl_idx, good_pixel_mask, noise_model_parameters, args)
